...
public class SetupController : Controller
{
    ...
    public SetupController(IApi api, IMenuService menuService)
    {
        _api = api;
    ...
    public async Task<IActionResult> Seed()
    {
        ...
        // Add docs page
        var docsPage = await GenericPage.CreateAsync(_api);
        ...
        // Add start page
        var startPage = await GenericPage.CreateAsync(_api);
        ...
        // Add blog posts
        var post1 = await BlogPost.CreateAsync(_api);
        ...
        var post2 = await BlogPost.CreateAsync(_api);
        ...
        var post3 = await BlogPost.CreateAsync(_api);
        ...
        // Menus
        await _menuService.SyncDefinitions(site.Id);
        var menus = (await _menuService.GetAll(site.Id)).Where(m => m.IsSystemDefined).ToDictionary(k => k.Slug);

        // Populate primary navigation if empty
        if (menus.TryGetValue("primary", out var primaryNav) && primaryNav.Items.Count == 0)
        {
            primaryNav.Items.Add(new LinkMenuItem
            {
                Id = Guid.NewGuid(),
                Link = new MenuLinkField
                {
                    Id = startPage.Id.ToString(),
                    Url = startPage.Permalink,
                    Text = startPage.NavigationTitle,
                    Type = LinkType.Page
                }
            });
            ...
            primaryNav.Items.Add(new LinkMenuItem
            {
                ...
                Link = new MenuLinkField
                {
                    Id = blogPage.Id.ToString(),
                    Url = blogPage.Permalink,
                    Text = blogPage.NavigationTitle,
                    Type = LinkType.Page
                }
            });
            ...
            primaryNav.Items.Add(new LinkMenuItem
            {
                ...
                Link = new MenuLinkField
                {
                    Id = docsPage.Id.ToString(),
                    Url = docsPage.Permalink,
                    Text = docsPage.NavigationTitle,
                    Type = LinkType.Page
                }
            });

            await _menuService.Save(primaryNav);
        }
